...
| Testnet SIMULATED yield pool | `TestYieldPool` 0x77b5…cA6A, `TestYieldAToken` 0x097F…afD3 | This repo (`src/testnet/TestYieldPool.sol`) | Demonstrate MonSave's yield mechanics on Monad Testnet where no real Aave market exists | `packages/config/src/tokens.ts` (`SIMULATED_YIELD`), circle Yield tab, create wizard | **Deployed on Testnet only, deliberately.** A fixed 8% *demo* rate on the valueless tUSD token, paid from a real 1M-token reserve, exercised through the REAL AaveV3MonadAdapter + circle yield code. **Labeled everywhere as "simulated test yield, not a real market/return."** Chain-guarded to 10143; never referenced by mainnet deploy. This intentionally overrides the brief's "no public simulated-yield contract" rule for testnet demonstration, at the owner's explicit request, with prominent honest labeling. Mainnet uses real Aave instead. |
